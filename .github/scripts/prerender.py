...
ZONAS = ['services-grid', 'gallery-grid', 'rigidos-grid', 'models-grid',
         ...
         # texto que o cliente edita no backoffice e que estava a congelar no
         # HTML servido: as 6 perguntas da FAQ, os 4 passos e a lista do B2B
         'faq-list', 'steps-grid', 'b2b-list']
...
CABECALHOS = [
    ('servicos', 'services.json', 'head'),
    ('trabalhos', 'gallery.json', 'head'),
    ('modelos', 'models.json', 'head'),
    ('rigidos', 'rigidos.json', 'head'),
    ('lonas', 'lonas.json', 'head'),
    ('como', 'steps.json', 'head'),
    ('faq', 'faq.json', 'head'),
    ('contacto', 'site.json', 'contacto'),
    ('b2b', 'site.json', 'b2b'),
]
...
def escrever_sitemaps(raiz):
    ...
    galeria = ler_json(os.path.join(raiz, 'data', 'gallery.json'), 'data/gallery.json')
...
def construir_jsonld(raiz):
    ...
    if not m_cp:
        raise ValueError(
            'contacts.address_line2 = %r não está no formato "4815-429 Vizela". '
    ...
    negocio = {
        ...
        'alternateName': ['Art Stamp Creations', 'ArtStamp Creations',
                          'ArtStamp', 'Art Stamp'],
    ...
    pagina = {
        ...
        '@id': SITE + '/',
        'url': SITE + '/',
        'name': 'ArtStampCreations — Personalização e Estampagem',
        'inLanguage': 'pt-PT',
